...
1. **Run with `bun run dev` (== `nd dev`) for hot reload + the crash-restart overlay.** This wraps
   ...
   iterating on the framework's host itself (`nd dev` runs the prebuilt native host for your
   platform — appkit on macOS, gtk on Linux — not an in-place host rebuild). **`.tsx`/`.desktop.tsx` component files must still import hooks from
