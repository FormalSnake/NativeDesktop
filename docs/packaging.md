...
flow verifies what it downloads. It also states, explicitly, what M9 does
**not** do — every deferral below is a deliberate scoping call, not an
oversight.

`nd package` is a documented convention, not a shipped binary (same
...
**Windows packaging (signed NSIS installer + winget manifest) lands with M7, which is not yet implemented.**
...
an AppImage with `appimagetool`, falling back to `mksquashfs` if
`appimagetool` isn't on `PATH` (it currently is not, in the nix devshell —
so CI and local runs exercise the squashfs fallback path).
...
**Scoping call (M9-D4): the M9 gate only lint-validates this manifest**
...
sources, then deep-signs inside-out (nested Mach-O first — `bun`,
...
**Ad-hoc + hardened runtime, the honest truth:** `--options runtime` *is*
...
**Notarization is gated on secrets, not disabled by policy.**
...
asserted behavior of `scripts/mac/mac-m9.sh` and the `macos-package` CI job
— it is not a bug and does not fail the gate.

**Mac machine prerequisite:** the Mac dev loop signs update archives with
`minisign`, which is **not** part of Xcode or the system toolchain — it
must be installed via `brew install minisign` on the Mac (the CI job installs
it as a step; a developer's machine needs it pre-installed, same as Zig/Bun).
...
- `delta` — **reserved, not populated in M9.** The schema leaves room for a
  ...
  manifest shape, not a breaking flag day.
...
zstd delta-chaining from a previous version is explicitly deferred** — it
...
zero I/O — it only ever operates on byte slices handed to it. There is
**no flag, environment variable, or manifest field that skips verification**:
...
- `macos-package` (stretch, **non-blocking** — `continue-on-error: true`,
  same convention as `.github/workflows/mac.yml`): installs Zig + Bun +
  ...
  merge. As with `mac.yml`, the first live run of this job is deferred —
  this authoring session has no GitHub push access — so it has been
  validated as lint-clean YAML locally rather than observed running on a
  real runner.
