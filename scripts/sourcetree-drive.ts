...
import { type Png, type Rect, decodePng, inkBandHeight, luminance, modalFill } from "./capture-ink.ts";
...
// For one row's scanline band, the contiguous column runs that contrast with
// the row fill, in logical units relative to the widget's left edge. Every
// piece of row content (each glyph of the title, the badge, the action button)
// is one run, so two states of the same rows can be compared column for column
// instead of by eye. The PNG reader and the ink primitives are in
// capture-ink.ts, shared with the other capture-measuring gates.
