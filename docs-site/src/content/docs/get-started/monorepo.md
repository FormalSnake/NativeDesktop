...
`@nativedesktop/react` declares `react` as a `peerDependency` (`^19.2.7`), not a vendored copy —
that single detail is what makes it possible for a NativeDesktop app to sit in a monorepo next to a
web (`react-dom`) app and a React Native app, and share a hooks/logic package with both, instead of
living in an isolated checkout of its own.
...
dependencies — a linked app and the package it linked could each resolve a *different* copy of
...
the deduping for free, and the same guarantee extends to *every* workspace member, not just the one
app the old script patched.
...
The Bun `onLoad` rewrite (the `nd dev`/`bun --hot` path) filters on `/\.ts$/` — deliberately
excluding `.tsx` and `.desktop.tsx`. This isn't an oversight: Bun's runtime `onLoad` has no
fall-through, a matched file **must** return contents, and once a plugin returns contents for a
file, `bun --hot` drops that file from its watch set. Intercepting a component file would silently
kill its hot reload. So the rewrite only ever touches shared, non-component `.ts` modules — exactly
where cross-platform hooks live.
...
`.desktop.tsx` is a platform **suffix**, not a custom file format — the same convention React
Native uses for `.native.tsx`. It's an ordinary `.tsx` file that TypeScript, ESLint, Prettier, and
...
`@nativedesktop/react` itself is npm-publishable now (`publishConfig.access: "public"`, a `files:
["dist"]` allowlist, a `postinstall` build step) — though it isn't published yet; every package in
this repo still links via `file:`/`workspace:` paths (see [Project Layout](/get-started/project-layout/)).
