...
When an app needs a widget the built-in set does not cover (a map view, a chart, a platform SDK's
own view, a piece of legacy AppKit or GTK code), it can ship that widget as a plugin without
rebuilding NativeDesktop. The host loads an app-owned shared library at launch, and the generic
`<nativeview>` widget routes create, update, command, and event traffic to a factory that library
registers.
...
`ND_PLUGINS=1`. `nd dev` and `nd build` set both from `native.plugins`; a packaged app's launch
script sets them to the bundled plugin paths under the app root. On startup, with `ND_PLUGINS=1`
set, the host calls `nd_load_plugins_from_env`, which splits `ND_PLUGIN_PATHS` on `:`, skips empty
segments, and calls `nd_load_plugin` on each path in order. Nothing loads a plugin unless the
embedder opts in this way, and no plugins directory is ever scanned.
...
`examples/nativeview-demo` is a working plugin: a `ColorView` that renders a solid color box,
forwards a `color` prop, emits a `pressed` event on click, and answers a `reset` command.
...
`NativeDesktopPlugin.registerView` from `init`. That path supports one view kind per plugin;
hand-roll an `nd_view_impl` to register more. `NativeDesktopSwiftUIView` hosts a SwiftUI tree behind
the same protocol, under the same ownership rule as the framework's own `SettingsGroup` bridge:
NativeDesktop retains one AppKit identity while SwiftUI controls the content inside it. Production
`.app` packaging has to embed the plugin dylib and include it in signing and notarization; the
development loader accepts an absolute build path.
