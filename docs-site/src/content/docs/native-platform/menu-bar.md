...
"where the app's commands live" — a real menu bar on macOS, a primary hamburger menu on GNOME —
rather than one widget drawn identically everywhere.
...
- **`iconName`** — a freedesktop name, the same vocabulary as `Button.iconName` (see
  [Icons](/native-platform/icons/)). Resolved through the same mac SF Symbol map; deliberately
  **not rendered on GNOME** — popover menus don't render item icons and the HIG discourages them,
  so this isn't a gap to fix, it's the platform-correct choice.
...
macOS installs the full standard default main menu — App, File, Edit, View, Window, Help, each
wired to the responder chain (`NSText` selectors, `NSApplication` actions) — at window creation,
**even when the tree has no `<menubar>` at all**. That's why `Edit > Copy`, `Cut`, `Paste`, `Select
All`, `Quit`, `Minimize`, and friends work in every text field automatically, with no app code:
they're responder-chain selectors with a nil target, not custom handlers.
...
declared menus are installed — no default File/Edit/View/Window/Help.

### GNOME: no top menubar, by design

GNOME Shell doesn't show a top-level menubar — that's correct GNOME design, not a missing feature.
Instead, `<menubar>` renders as a primary menu button (`GtkMenuButton`, `open-menu-symbolic`,
...
labels as their text, with nominal (non-visual) bounds, since they're chrome rather than an
...
- `iconName` on a `<menuitem>` has zero visible effect on GNOME — this is intentional, not a
  todo. Don't rely on the icon for information the label doesn't already carry.
- Only one merge rule exists for mac (append after separator on a title match, otherwise insert
  before Window) — there's no way to target Edit/View/Help insertion order beyond that rule today.
